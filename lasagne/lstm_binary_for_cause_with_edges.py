...
def vectorize_path(path_words_list, path_label_list, word_idx, path_maxlen):
	data_size = len(path_words_list)
	paths = np.zeros((data_size, path_maxlen), dtype=np.int32)
	masks = np.zeros((data_size, path_maxlen), dtype=np.float32)
	labels = np.zeros((data_size), dtype=np.int32)
	for i in range(len(path_words_list)):
		path_words = path_words_list[i]
		for j, w in enumerate(path_words):
			if j >= path_maxlen:
				break
			paths[i][j] = word_idx[w]
		masks[i] = np.concatenate((np.ones(min(len(path_words), path_maxlen), dtype=np.float32),\
									np.zeros(max(0, path_maxlen - len(path_words)), dtype=np.float32)))
		labels[i] = path_label_list[i]
	return paths, masks, labels
...
if __name__ == '__main__':
	...
	for idx in range(len(word_embeddings)):
		...
		if np.all(word_embeddings[idx]==0) or not np.all(np.isnan(word_embeddings[idx])==False):
			embedding_weights[idx,:] = uniform_sample(-0.25,0.25,WORD_EMBEDDING_DIM)
	...
	log_file = 'logs/lstm_cause_notFreeze_withedges.txt'
	log = open(log_file, 'w')
	...
	preds = np.argmax(preds, axis=1)
	print classification_report(labels, preds)
	p.dump(preds, open(os.path.join(os.path.dirname(sys.argv[3]), "data_predicted_cause_labels.p"), 'wb'))
